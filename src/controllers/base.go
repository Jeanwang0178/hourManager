...
func (this *BaseController) auth() {
	arr := strings.Split(this.Ctx.GetCookie("token"), "|")
	if len(arr) == 2 {
		idstr, password := arr[0], arr[1]
		userId := idstr
		if len(userId) > 0 {
			user, err := services.UserServiceUserGetById(userId)
	...
	if len(this.userId) == 0 && (this.controllerName != "default" ||
		(this.controllerName == "default" && this.actionName != "logout" && this.actionName != "login" && this.actionName != "postLogin")) {
